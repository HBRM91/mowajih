  ...
  return (
    <AnimatePresence>
      {lastBadgeEarned && (
        ...
        >
          <span className="text-3xl">{BADGE_EMOJIS[lastBadgeEarned] ?? "🏅"}</span>
          <div>
            <div className="text-xs text-gold-300 uppercase tracking-wider font-semibold">{t("badge.new_label")}</div>
            <div className="font-heading font-bold">{t(`badge.${lastBadgeEarned}`)}</div>
            <div className="text-xs text-gold-200/70">{t("gamification.xp_bonus")}</div>
          </div>
          <button type="button" aria-label="Fermer" onClick={dismissBadge} className="ml-2 text-white/50 hover:text-white">
