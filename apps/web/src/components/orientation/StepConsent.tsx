...
export default function StepConsent({ onSubmit, isLoading }: { onSubmit: (token?: string) => void; isLoading: boolean }) {
  ...
  const { consent, setField, prevStep } = useFormStore();
  ...
  return (
    <div>
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-8">
        <span className="text-gold-600 text-sm font-semibold uppercase tracking-wider">Étape 4 / 4</span>
        <h2 className="font-heading text-3xl font-bold text-navy-800 mt-2">{t("step.consent")}</h2>
        <p className="text-navy-400 mt-2">Dernière étape avant de découvrir tes opportunités.</p>
      </motion.div>
      ...
      <div className="bg-gradient-to-br from-navy-50 to-navy-100/30 p-6 rounded-2xl border border-navy-200/30 mb-5">
        ...
        <div className="bg-white/60 rounded-xl p-4 text-xs text-navy-400 space-y-2">
          <div className="flex items-center gap-2">
            ...
            <span>Conservation limitée à 24 mois</span>
          </div>
          <div className="flex items-center gap-2">
            <svg className="w-4 h-4 text-emerald-500 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
            </svg>
            <span>Toutes les actions sont journalisées (CNDP)</span>
          </div>
          <div className="flex items-center gap-2">
            <svg className="w-4 h-4 text-emerald-500 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
            </svg>
            <span>Droit à la suppression à tout moment</span>
          </div>
        </div>
      </div>
